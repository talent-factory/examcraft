...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('role_id', sa.String(255), sa.ForeignKey('rbac_roles.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('feature_id', sa.String(255), sa.ForeignKey('features.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
    )
    op.create_index('idx_role_feature_unique', 'role_features', ['role_id', 'feature_id'], unique=True)
    ...
    op.create_table(
        'tier_quotas',
        sa.Column('id', sa.Integer, primary_key=True, autoincrement=True),
        sa.Column('tier_id', sa.String(255), sa.ForeignKey('subscription_tiers.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('resource_type', sa.String(100), nullable=False, index=True),
        sa.Column('quota_limit', sa.Integer, nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now())
    )
    op.create_index('idx_tier_quota_unique', 'tier_quotas', ['tier_id', 'resource_type'], unique=True)
    ...
    op.create_index('idx_tier_feature_unique', 'tier_features', ['tier_id', 'feature_id'], unique=True)
    ...
    op.create_table(
        ...
        sa.Column('tier_id', sa.String(255), sa.ForeignKey('subscription_tiers.id'), nullable=True, index=True),
        sa.Column('subscription_status', sa.String(50), nullable=False, server_default='active', index=True),
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now(), onupdate=sa.func.now()),
        sa.CheckConstraint("subscription_status IN ('active', 'suspended', 'cancelled', 'trial')", name='check_subscription_status')
    )
    ...
    op.create_index('idx_resource_usage_unique', 'resource_usage', ['organization_id', 'resource_type', 'period_start'], unique=True)
    ...
    op.create_index('idx_audit_user_action', 'permission_audit_log', ['user_id', 'action'])
