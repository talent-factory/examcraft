...
def upgrade() -> None:
    # Add institution_id column to question_reviews table
    op.add_column(
        "question_reviews", sa.Column("institution_id", sa.Integer(), nullable=True)
    )
    ...
    op.create_index(
        "ix_question_reviews_institution_id", "question_reviews", ["institution_id"]
    )
...
def downgrade() -> None:
    ...
    # Drop foreign key
    op.drop_constraint(
        "fk_question_reviews_institution_id", "question_reviews", type_="foreignkey"
    )
