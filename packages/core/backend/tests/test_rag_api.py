...
class TestRAGAPI:
    ...
    def test_generate_rag_exam_success(self, sample_rag_response, mock_processed_document):
        ...
        with patch('api.rag_exams.document_service') as mock_doc_service, \
            ...
            # Mock RAG Service
            mock_rag_service.generate_rag_exam = AsyncMock(return_value=sample_rag_response)
    ...
    def test_generate_rag_exam_validation_errors(self):
        ...
        # Leeres Topic
        response = client.post("/api/v1/rag/generate-exam", json={
            "topic": "",  # Zu kurz
            "question_count": 1
        })
    ...
    def test_generate_rag_exam_service_error(self, mock_processed_document):
        ...
        with patch('api.rag_exams.document_service') as mock_doc_service, \
            ...
            mock_doc_service.get_document_by_id.return_value = mock_processed_document
            mock_rag_service.generate_rag_exam = AsyncMock(side_effect=Exception("Service Error"))
    ...
    def test_retrieve_context_validation_errors(self):
        ...
        # Leere Query
        response = client.post("/api/v1/rag/retrieve-context", json={
            "query": ""  # Zu kurz
        })
        assert response.status_code == 422
    ...
    def test_get_available_documents_all(self, mock_processed_document):
        ...
        with patch('api.rag_exams.document_service') as mock_doc_service:
            ...
            response = client.get("/api/v1/rag/available-documents?processed_only=false")
    ...
    def test_rag_service_health_healthy(self):
        ...
        with patch('api.rag_exams.vector_service') as mock_vector_service, \
            ...
            mock_rag_service.question_templates = {"mc": "template", "oe": "template", "tf": "template"}
    ...
    def test_rag_service_health_unhealthy(self):
        ...
        with patch('api.rag_exams.vector_service') as mock_vector_service:
            mock_vector_service.get_collection_stats.side_effect = Exception("Vector Service Error")
...
class TestRAGAPIIntegration:
    ...
    def test_full_rag_workflow_mock(self):
        ...
        with patch('api.rag_exams.rag_service') as mock_rag_service:
            ...
            context_response = client.post("/api/v1/rag/retrieve-context", json={
                "query": "Integration Test",
                "document_ids": [1],
                "max_chunks": 3
            })
        ...
        with patch('api.rag_exams.document_service') as mock_doc_service, \
            ...
            mock_rag_service.generate_rag_exam = AsyncMock(return_value=mock_exam_response)
            
            exam_response = client.post("/api/v1/rag/generate-exam", json={
                "topic": "Integration Test",
                "document_ids": [1],
                "question_count": 1,
