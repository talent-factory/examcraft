...
def rbac_db(test_db):
    ...
    quotas = [
        ...
        TierQuota(tier_id="tier_free", resource_type="questions_per_month", quota_limit=20),
        TierQuota(tier_id="tier_pro", resource_type="documents", quota_limit=-1),  # Unlimited
        TierQuota(tier_id="tier_pro", resource_type="questions_per_month", quota_limit=1000)
    ]
    for quota in quotas:
        test_db.add(quota)
...
def test_user_has_feature_access_with_role_and_tier(rbac_db):
    ...
    assert not service.user_has_feature_access(user.id, "test_management", log_access=False)
...
def test_check_resource_quota_unlimited(rbac_db):
    ...
    from models.rbac import Organization
    org = Organization(
...
def test_update_role_features(rbac_db):
    ...
    updated_role = service.update_role_features(
        role.id,
        ["feat_test_gen", "feat_test_mgmt", "feat_admin"]
    )
...
def test_list_roles(rbac_db):
    ...
    custom_roles = service.list_roles(include_system_roles=False, include_inactive=False)
