...
def test_log_register(db):
    ...
    audit_log = AuditService.log_register(
        db=db,
        user_id=user.id,
        email=user.email
    )
...
def test_log_rate_limit_exceeded(db):
    ...
    audit_log = AuditService.log_rate_limit_exceeded(
        db=db,
        limit_type="ip"
    )
...
def test_query_audit_logs_by_action(db):
    ...
    login_logs = db.query(AuditLog).filter(AuditLog.action == AuditService.ACTION_LOGIN).all()
