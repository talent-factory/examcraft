...
class TestDoclingProcessorDocumentProcessing:
    ...
    @pytest.mark.asyncio
    async def test_process_document_with_tables(self, docling_processor, sample_pdf_path):
        """Test: Document Processing mit Tabellen"""
        with patch.object(docling_processor.converter, 'convert') as mock_convert:
            # Mock Table
            mock_table = MagicMock()
            mock_table.export_to_markdown.return_value = "| Header | Value |\n|--------|-------|\n| A | 1 |"
    ...
    async def test_large_document_chunking(self, docling_processor, sample_pdf_path):
        ...
        with patch.object(docling_processor.converter, 'convert') as mock_convert:
            ...
            for chunk in result.chunks:
                ...
                assert word_count <= docling_processor.chunk_size + docling_processor.chunk_overlap
