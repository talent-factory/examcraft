...
class TestQuestionReviewAPI:
    ...
    def test_get_question_review_success(
        self,
        mock_get_review,
        client,
        mock_question_review,
        mock_review_comment,
        mock_review_history,
    ):
    ...
    def test_get_question_review_not_found(self, mock_get_review, client):
        ...
        mock_get_review.side_effect = HTTPException(
            status_code=404, detail="Question review not found"
        )
    ...
    def test_create_question_review_success(
        self, mock_create, client, mock_question_review
    ):
    ...
    def test_reject_question_success(self, mock_reject, client, mock_question_review):
        ...
        request_data = {"reviewer_id": "reviewer_1", "reason": "Question is ambiguous"}
    ...
    def test_edit_question_success(self, mock_edit, client, mock_question_review):
        ...
        request_data = {"question_text": "Updated question text", "difficulty": "hard"}

        response = client.put(
            "/api/v1/questions/1/edit?editor_id=teacher_1", json=request_data
        )
    ...
    def test_get_review_history_success(
        self, mock_get_history, client, mock_review_history
    ):
