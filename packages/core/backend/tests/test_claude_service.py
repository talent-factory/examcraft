...
class TestClaudeService:
    ...
    async def test_api_request_with_retry_success(self, claude_service):
        ...
        with patch.object(claude_service, '_make_api_request_with_retry', return_value=mock_response):
            with patch.object(claude_service, '_parse_claude_response', return_value=[{"id": 1, "question": "Test"}]):
                questions = await claude_service.generate_questions("Python", "medium", 1)
    ...
    async def test_fallback_to_demo_on_api_failure(self, claude_service):
        ...
        with patch.object(claude_service, '_make_api_request_with_retry', side_effect=Exception("API Error")):
            with patch.object(claude_service, '_generate_demo_questions', return_value=[{"id": 1, "question": "Demo"}]):
                questions = await claude_service.generate_questions("Python", "medium", 1)
