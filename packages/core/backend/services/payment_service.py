...
class PaymentService:
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning(
                "⚠️ STRIPE_SECRET_KEY not found in environment variables. Payment features will be disabled."
            )
        else:
        ...
        try:
            ...
            return {
                ...
                "plan": {
                    ...
                    "currency": subscription.items.data[0].price.currency.upper() if subscription.items.data else "CHF",
                    "interval": subscription.items.data[0].price.recurring.interval if subscription.items.data else None,
                },
                "default_payment_method": self._format_payment_method(subscription.default_payment_method) if subscription.default_payment_method else None,
            }
        except stripe.error.StripeError as e:
            logger.error(f"Stripe Error getting subscription: {str(e)}")
            raise e

    async def get_invoices(self, stripe_customer_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
            ...
            return [
                {
                    ...
                    "paid_at": datetime.fromtimestamp(invoice.status_transitions.paid_at).isoformat() if invoice.status_transitions and invoice.status_transitions.paid_at else None,
    ...
    async def get_payment_methods(self, stripe_customer_id: str) -> List[Dict[str, Any]]:
        """
    ...
    def _format_payment_method(self, pm: Any) -> Dict[str, Any]:
        ...
        if isinstance(pm, str):
            # If it's just an ID, we can't format it
            return {"id": pm}
