...
class ReviewService:
    ...
    def get_pending_reviews(
        ...
        return query.order_by(QuestionReview.created_at.desc()).limit(limit).offset(offset).all()
    ...
    def approve_question(
        ...
        """
        question = self.db.query(QuestionReview).filter(QuestionReview.id == question_id).first()
    ...
    def add_comment(
        ...
        return self._add_comment(question_id, comment_text, comment_type, author, author_role)
    ...
    def get_review_statistics(self) -> Dict[str, int]:
        ...
        pending = self.db.query(QuestionReview).filter(
            QuestionReview.review_status == ReviewStatus.PENDING.value
        ).count()
        approved = self.db.query(QuestionReview).filter(
            QuestionReview.review_status == ReviewStatus.APPROVED.value
        ).count()
        rejected = self.db.query(QuestionReview).filter(
            QuestionReview.review_status == ReviewStatus.REJECTED.value
        ).count()
        edited = self.db.query(QuestionReview).filter(
            QuestionReview.review_status == ReviewStatus.EDITED.value
        ).count()
        in_review = self.db.query(QuestionReview).filter(
