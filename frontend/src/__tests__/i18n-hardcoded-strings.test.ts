 ...
 * progress by. As of TF-772 PR 3 the gap costs nothing currently outstanding:
 * `DocumentUpload.tsx`/`DocumentLibrary.tsx` already had their two literals
 * removed (`Upload cancelled` became the `UploadCancelled` sentinel, and the
 * `Document processing failed…`/`…timeout after…` strings went with the
 * deleted dead `waitForDocumentProcessing` function), and this PR migrates
 * the third — `ResendVerificationButton.tsx`'s `Failed to resend verification
