...
def seed_features(db):
    ...
    features = [
        ...
        (
            ...
            "Generierung von Prüfungsfragen mit Claude AI",
            "generation",
        ),
        (
            "feat_doc_upload",
            "document_upload",
            "Dokumente hochladen",
            "Upload von PDF, DOCX, Markdown Dokumenten",
            "management",
        ),
        (
            "feat_doc_library",
            ...
            "Verwaltung und Durchsuchung von Dokumenten",
            "management",
        ),
        (
            "feat_rag_gen",
            ...
            "RAG-basierte Prüfungserstellung aus Dokumenten",
            "generation",
        ),
        (
            "feat_chatbot",
            "document_chatbot",
            "Dokument ChatBot",
            "Interaktive Konversation mit Dokumenten (NotebookLM-Style)",
            "generation",
        ),
        (
            "feat_review",
            "question_review",
            "Question Review",
            "Review und Approval Workflow für generierte Fragen",
            "management",
        ),
        (
            "feat_prompts",
            "prompt_management",
            "Prompt Management",
            "Verwaltung der Prompt Knowledge Base",
            "administration",
        ),
        (
            "feat_users",
        ...
        (
            ...
            "Konfiguration von System-Einstellungen",
            "administration",
        ),
        (
            "feat_analytics",
...
def seed_rbac_roles(db):
    ...
    roles = [
        ...
        (
            ...
            "dozent",
            "Dozent",
            "Erstellt Prüfungen, verwaltet Dokumente, nutzt KI-Features",
            True,
        ),
        (
            "role_assistant",
            "assistant",
            "Assistent",
            "Unterstützt bei Prüfungserstellung, limitierter Zugriff",
            True,
        ),
        (
            "role_viewer",
...
def seed_role_features(db):
    ...
    for feat_id in dozent_features:
        existing = (
            db.query(RoleFeature)
            .filter(
                RoleFeature.role_id == "role_dozent", RoleFeature.feature_id == feat_id
            )
            .first()
        )
        if not existing:
    ...
    for feat_id in viewer_features:
        existing = (
            db.query(RoleFeature)
            .filter(
                RoleFeature.role_id == "role_viewer", RoleFeature.feature_id == feat_id
            )
            .first()
        )
        if not existing:
...
def seed_tier_quotas(db):
    ...
    for tier_id, resource_type, quota_limit in quotas:
        existing = (
            db.query(TierQuota)
            .filter(
                TierQuota.tier_id == tier_id, TierQuota.resource_type == resource_type
            )
            .first()
        )
        if existing:
...
def seed_tier_features(db):
    ...
    for feat_id in free_features:
        existing = (
            ...
            .filter(
                TierFeature.tier_id == "tier_free", TierFeature.feature_id == feat_id
            )
            .first()
        )
        if not existing:
    ...
    # Starter: Free + Review
    starter_features = free_features + ["feat_review"]
    for feat_id in starter_features:
        existing = (
            db.query(TierFeature)
            .filter(
                TierFeature.tier_id == "tier_starter", TierFeature.feature_id == feat_id
            )
            .first()
        )
        if not existing:
