"""Diagnostics tasks (TF-359, TF-865, TF-868).
...
``/api/specula-test/*`` endpoints only exercise the API process and are
...
Triggered exclusively by the SuperAdmin-gated ``POST /api/admin/specula-test/
...
class SpeculaPipelineTestError(RuntimeError):
...
def trigger_test_error(self, user_id: int, message: str | None = None) -> None:
    ...
    raise SpeculaPipelineTestError(detail)
