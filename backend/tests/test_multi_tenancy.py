...
def test_users_belong_to_institutions(db):
    ...
    institution1 = db.query(Institution).filter(Institution.slug == "university-a").first()
    institution2 = db.query(Institution).filter(Institution.slug == "university-b").first()
...
def test_question_tenant_isolation(db):
    ...
    filtered_query1 = TenantFilter.filter_by_tenant(query1, QuestionReview, tenant_context1)
    ...
    filtered_query2 = TenantFilter.filter_by_tenant(query2, QuestionReview, tenant_context2)
...
def test_superuser_can_see_all_data(db):
    ...
    filtered_query = TenantFilter.filter_by_tenant(query, Document, tenant_context, allow_superuser_access=True)
...
def test_user_limit_enforcement(db):
    ...
    institution = db.query(Institution).filter(Institution.slug == "university-a").first()
...
def test_professional_tier_has_higher_limits(db):
    ...
    free_institution = db.query(Institution).filter(Institution.slug == "university-a").first()
    pro_institution = db.query(Institution).filter(Institution.slug == "university-b").first()
    ...
    assert pro_institution.max_questions_per_month > free_institution.max_questions_per_month
