...
class TestVectorSearchAPI:
    ...
    def test_vector_search_health_failure(self, client):
        ...
        with patch('api.vector_search.vector_service') as mock_service:
            ...
            assert response.status_code == 200  # Health endpoint returns 200 even on errors
            data = response.json()
    ...
    def test_similarity_search_success(self, mock_doc_service, mock_vector_service, client, sample_search_results):
    ...
    @patch('api.vector_search.vector_service')
    @patch('api.vector_search.document_service')
    def test_similarity_search_with_document_filter(self, mock_doc_service, mock_vector_service, client):
    ...
    def test_similarity_search_invalid_query(self, client):
        ...
        # Ungültige n_results
        response = client.post("/api/v1/search/similarity", json={
            "query": "test",
            "n_results": 0
        })
        assert response.status_code == 422
    ...
    def test_similarity_search_service_error(self, mock_vector_service, client):
        ...
        mock_vector_service.similarity_search = AsyncMock(side_effect=Exception("Vector search failed"))
    ...
    @patch('api.vector_search.vector_service')
    @patch('api.vector_search.document_service')
    def test_get_document_vector_chunks_success(self, mock_doc_service, mock_vector_service, client, sample_search_results):
        ...
        # Mock Vector Service
        mock_vector_service.get_document_chunks = AsyncMock(return_value=sample_search_results)
    ...
    @patch('api.vector_search.vector_service')
    @patch('api.vector_search.document_service')
    def test_get_document_vector_chunks_service_error(self, mock_doc_service, mock_vector_service, client):
        """Test Document Vector Chunks Service Error"""
        mock_document = Mock()
        mock_doc_service.get_document_by_id.return_value = mock_document
        mock_vector_service.get_document_chunks = AsyncMock(side_effect=Exception("Vector service error"))
    ...
    def test_get_vector_database_stats_error(self, mock_vector_service, client):
        ...
        mock_vector_service.get_collection_stats.side_effect = Exception("Stats unavailable")
    ...
    @patch('api.vector_search.vector_service')
    @patch('api.vector_search.document_service')
    def test_delete_document_vectors_success(self, mock_doc_service, mock_vector_service, client):
    ...
    @patch('api.vector_search.vector_service')
    @patch('api.vector_search.document_service')
    def test_reindex_document_vectors_success(self, mock_doc_service, mock_vector_service, client):
        ...
        mock_doc_service.process_document_with_vectors = AsyncMock(return_value=mock_processing_result)
    ...
    @patch('api.vector_search.vector_service')
    @patch('api.vector_search.document_service')
    def test_reindex_document_processing_failed(self, mock_doc_service, mock_vector_service, client):
