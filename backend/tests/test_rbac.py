...
def db(test_db):
    ...
    roles = [
        ...
        Role(
            ...
            permissions=[
                "create_questions", "approve_questions", "edit_questions",
                "create_documents", "delete_documents", "view_questions"
            ],
            is_system_role=True
        ),
        Role(
...
def test_user_with_multiple_roles(db):
    ...
    assistant_role = db.query(Role).filter(Role.name == UserRole.ASSISTANT.value).first()
