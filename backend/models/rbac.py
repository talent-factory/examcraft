...
from sqlalchemy import (
    ...
    Text, DECIMAL, CheckConstraint, Index
)
...
class Feature(Base):
    ...
    name = Column(String(100), nullable=False, unique=True, index=True)  # question_generation_ai
    display_name = Column(String(200), nullable=False)  # "KI-Prüfung erstellen"
    description = Column(Text, nullable=True)
    category = Column(String(50), nullable=True)  # generation, management, administration, integration
    is_active = Column(Boolean, default=True, nullable=False, index=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    
    # Relationships
    role_features = relationship("RoleFeature", back_populates="feature", cascade="all, delete-orphan")
    ...
    __table_args__ = (
        CheckConstraint(
            "name ~ '^[a-z0-9_]+$'",
            name='check_feature_name_format'
        ),
    )
...
class RoleFeature(Base):
    ...
    role_id = Column(String(255), ForeignKey('rbac_roles.id', ondelete='CASCADE'), nullable=False, index=True)
    feature_id = Column(String(255), ForeignKey('features.id', ondelete='CASCADE'), nullable=False, index=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    ...
    def __repr__(self):
        return f"<RoleFeature(role_id='{self.role_id}', feature_id='{self.feature_id}')>"
...
class RBACRole(Base):
    ...
    id = Column(String(255), primary_key=True)  # role_admin, role_dozent, role_custom_xyz
    name = Column(String(100), nullable=False, unique=True, index=True)  # admin, dozent, custom_xyz
    display_name = Column(String(200), nullable=False)
    description = Column(Text, nullable=True)
    is_system_role = Column(Boolean, default=False, nullable=False)  # System-Rollen können nicht gelöscht werden
    is_active = Column(Boolean, default=True, nullable=False, index=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    ...
    # Relationships
    role_features = relationship("RoleFeature", back_populates="role", cascade="all, delete-orphan")
    
    # Constraints
    __table_args__ = (
        CheckConstraint(
            "name ~ '^[a-z0-9_]+$'",
            name='check_rbac_role_name_format'
        ),
    )
...
class SubscriptionTier(Base):
    ...
    id = Column(String(255), primary_key=True)  # tier_free, tier_starter, tier_professional, tier_enterprise
    name = Column(String(50), nullable=False, unique=True, index=True)  # free, starter, professional, enterprise
    display_name = Column(String(100), nullable=False)  # "Free", "Starter", "Professional", "Enterprise"
    description = Column(Text, nullable=True)
    price_monthly = Column(DECIMAL(10, 2), nullable=True)  # 0.00, 19.00, 49.00, 149.00
    price_yearly = Column(DECIMAL(10, 2), nullable=True)  # 0.00, 190.00, 490.00, 1490.00
    is_active = Column(Boolean, default=True, nullable=False, index=True)
    ...
    # Relationships
    tier_quotas = relationship("TierQuota", back_populates="tier", cascade="all, delete-orphan")
...
class TierQuota(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    tier_id = Column(String(255), ForeignKey('subscription_tiers.id', ondelete='CASCADE'), nullable=False, index=True)
    resource_type = Column(String(100), nullable=False, index=True)  # documents, questions_per_month, users, storage_mb
...
class TierFeature(Base):
    ...
    def __repr__(self):
        return f"<TierFeature(tier_id='{self.tier_id}', feature_id='{self.feature_id}')>"
...
class Organization(Base):
    ...
    tier_id = Column(String(255), ForeignKey('subscription_tiers.id'), nullable=True, index=True)
    subscription_status = Column(String(50), default='active', nullable=False, index=True)  # active, suspended, cancelled
    ...
    resource_usage = relationship("ResourceUsage", back_populates="organization", cascade="all, delete-orphan")
...
class ResourceUsage(Base):
    ...
    resource_type = Column(String(100), nullable=False, index=True)  # documents, questions_per_month, storage_mb
    ...
    # Constraints
    __table_args__ = (
        Index('idx_resource_usage_unique', 'organization_id', 'resource_type', 'period_start', unique=True),
    )
...
class PermissionAuditLog(Base):
    ...
    action = Column(String(100), nullable=False, index=True)  # access_granted, access_denied, role_changed, feature_accessed
    resource_type = Column(String(100), nullable=True, index=True)  # feature, role, organization
