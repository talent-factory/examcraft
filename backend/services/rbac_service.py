...
class RBACService:
    ...
    def user_has_feature_access(
        ...
        for role in user.roles:
            ...
            role_feature = self.db.query(RoleFeature).filter(
                and_(
        ...
        if not role_has_feature:
            if log_access:
                self._log_access_denied(user_id, 'feature', feature_name, 'role_permission_denied')
            return False
        ...
        if user.institution_id:
            ...
            if institution and institution.subscription_tier:
                ...
                tier_has_feature = self.db.query(TierFeature).filter(
                    and_(
                ...
                if not tier_has_feature:
                    if log_access:
                        self._log_access_denied(user_id, 'feature', feature_name, 'tier_permission_denied')
                    return False
    ...
    def check_resource_quota(
        ...
        tier_id = f"tier_{institution.subscription_tier}"
        tier_quota = self.db.query(TierQuota).filter(
            and_(
        ...
        if not tier_quota:
            return {'allowed': True, 'reason': 'no_quota_defined'}  # Kein Limit = erlaubt
        ...
        usage = self.db.query(ResourceUsage).filter(
            and_(
    ...
    def list_roles(
        ...
        if not include_system_roles:
            query = query.filter(RBACRole.is_system_role == False)
        
        if not include_inactive:
            query = query.filter(RBACRole.is_active == True)
    ...
    def get_role_features(self, role_id: str) -> List[Feature]:
        ...
        """
        return self.db.query(Feature).join(RoleFeature).filter(
            and_(
    ...
    def _log_access_denied(self, user_id: int, resource_type: str, resource_id: str, reason: str):
        log = PermissionAuditLog(
            user_id=str(user_id),
