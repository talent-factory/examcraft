...
class DoclingProcessor:
    ...
    def __init__(self, chunk_size: int = 1000, chunk_overlap: int = 200):
        ...
        self.pipeline_options.do_ocr = (
            False  # Deaktiviert OCR (verhindert Model-Download)
        )
    ...
    async def process_document(
        ...
        try:
            ...
            chunks = self._create_semantic_chunks(
                result.document, markdown_content, metadata
            )
    ...
    def _extract_metadata(
        ...
        try:
            ...
            # ===== TABELLEN =====
            metadata["has_tables"] = (
                len(document.tables) > 0 if hasattr(document, "tables") else False
            )
            metadata["table_count"] = (
                len(document.tables) if hasattr(document, "tables") else 0
            )

            # ===== BILDER =====
            metadata["has_images"] = (
                len(document.images) > 0 if hasattr(document, "images") else False
            )
            metadata["image_count"] = (
                len(document.images) if hasattr(document, "images") else 0
            )
            ...
            if not sections and markdown:
                ...
                for match in matches:
                    ...
                    sections_with_hierarchy.append({"title": title, "level": level})
    ...
    def _create_semantic_chunks(
        self, document, markdown: str, metadata: Dict[str, Any]
    ) -> List[DocumentChunk]:
        """
