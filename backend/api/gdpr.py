...
async def export_user_data(
    ...
    try:
        ...
        try:
            ...
            questions = db.query(QuestionReview).filter(QuestionReview.created_by == current_user.id).all()
            user_data["questions"] = [
                {
                    "id": q.id,
        ...
        audit_logs = db.query(AuditLog).filter(AuditLog.user_id == current_user.id).limit(1000).all()
...
async def request_account_deletion(
    ...
    try:
        ...
        if hasattr(current_user, 'deletion_requested_at') and current_user.deletion_requested_at:
            raise HTTPException(
                status_code=400,
                detail="Account deletion already pending"
            )
...
async def cancel_account_deletion(
    ...
    """
    try:
        logger.info(f"Account deletion cancellation requested by user: {current_user.email}")
        
        # Check if deletion is pending
        if not hasattr(current_user, 'deletion_requested_at') or not current_user.deletion_requested_at:
            raise HTTPException(
                status_code=400,
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Deletion cancellation failed: {str(e)}")
...
async def delete_account_immediately(
    ...
    """
    try:
        logger.warning(f"Immediate account deletion requested by user: {current_user.email}")
        ...
        try:
            from models.question_review import QuestionReview
            db.query(QuestionReview).filter(QuestionReview.created_by == current_user.id).delete()
        except Exception as e:
